...
Dado que el enfoque empleado es Max-Min, se puede asumir que se ha utilizado el \textit{Modo A - FATI} (\textit{First Aggregate Then Infer}), que como su nombre indica, consiste en primero agregar los conjuntos difusos individuales inferidos, después defuzzificar para obtener un valor preciso, manteniendo toda la información difusa hasta el último paso \parencite{peregrin2000integracion}.

Para el proceso de desfuzzificación, en el estudio no se concreta de manera explícita el método utilizado, pero es muy probable que se haya empleado el método \textit{centroide}, también conocido como centro de gravedad, ya que es uno de los más comunes asociados al método Mamdani tradicional. 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.23\textwidth]{imgs/centroide.JPG}
